<?php
$pageTitle = 'Sync Gider Faturaları';
...
<div class="glass-card rounded-2xl p-6 max-w-5xl mx-auto mt-6">
    <div class="mb-8 text-center border-b border-slate-700/50 pb-6">
        <h2 class="text-3xl font-bold text-white mb-2">Gider Faturası Senkronizasyon Merkezi</h2>
        <p class="text-slate-400">Paraşüt'teki gider faturalarını Zoho CRM'e toplu olarak aktarın.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-slate-900/40 p-5 rounded-xl border border-slate-800">
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <select id="batchSize"
                class="bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-500/50 outline-none transition-all">
                <option value="50">50 Gider Faturası</option>
                <option value="100">100 Gider Faturası</option>
                <option value="250">250 Gider Faturası</option>
                <option value="500">500 Gider Faturası</option>
                <option value="1000">1000 Gider Faturası</option>
            </select>
        </div>
...
<script>
    ...
    async function refreshStats() {
        ...
        try {
            ...
            params.append('action', 'get_unsynced_purchase_orders_ids');
    ...
    function updateProgressUI() {
        ...
        // Batch Progress
        const batchPercent = currentBatchPOs.length > 0 ? Math.round((currentBatchIndex / currentBatchPOs.length) * 100) : 0;
    ...
    function pauseSync() {
        isPaused = true;
        log('Süreç duraklatıldı. Mevcut gider faturasından sonra duracak.', 'warning');
    ...
    async function fetchAndProcessBatch() {
        ...
        try {
            ...
            if (res.data.success && res.data.data && res.data.data.length > 0) {
                currentBatchPOs = res.data.data;
                currentBatchIndex = 0;
                log(`${currentBatchPOs.length} gider faturası alındı, işleme başlanıyor.`, 'info');
                processNextInBatch();
            } else {
                log('İşlenecek gider faturası kalmadı!', 'success');
                isSyncing = false;
                resetUI();
    ...
    async function processNextInBatch() {
        ...
        if (currentBatchIndex >= currentBatchPOs.length) {
            log('Paket tamamlandı. Bir sonraki paket yükleniyor...', 'batch');
            setTimeout(fetchAndProcessBatch, 1000);
            return;
        }

        const po = currentBatchPOs[currentBatchIndex];
        document.getElementById('batchStatus').textContent = `Gider Faturası #${po.invoice_number} Aktarılıyor...`;

        try {
            const params = new URLSearchParams();
            params.append('action', 'export_purchase_order_to_zoho');
            ...
            if (res.data.success) {
                successCount++;
                log(`Başarılı: #${po.invoice_number}`, 'success');
            } else {
                errorCount++;
                log(`Hata: #${po.invoice_number} - ${res.data.message}`, 'error');
            }
        } catch (e) {
            errorCount++;
            log(`Bağlantı Hatası: #${po.invoice_number}`, 'error');
        }
        ...
        if (isSyncing && !isPaused) {
            setTimeout(processNextInBatch, 300);
        }
    }
</script>
